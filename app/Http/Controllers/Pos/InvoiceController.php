...
class InvoiceController extends Controller
{
    ...
    {
        $allData = Invoice::with('user')->orderBy('date', 'desc')->orderBy('id', 'desc')->where('status', '1')->get();
    ...
    public function invoiceReportCsv($id)
    {
        $invoice = Invoice::with(['invoice_details.category', 'invoice_details.product', 'user'])->findOrFail($id);
        ...
        $csvData = [
            ...
            ['Customer Mobile', $payment->customer->mobile_no],
    ...
    public function dailyInvoiceReportCsv(Request $request)
    {
        $start_date = $request->start_date;
        $end_date = $request->end_date;

        $allData = Invoice::with(['payment.customer', 'user'])
            ->whereBetween('date', [$start_date, $end_date])
            ->get();

        $csvData = [
            ['Sl', 'Customer Name', 'Invoice No', 'Date', 'Description', 'Amount', 'Created by'],
        ];
        ...
        $filename = "daily_invoice_report_" . date('YmdHis') . ".csv";
        $handle = fopen($filename, 'w+');
