...
<div class="page-content">
    <div class="container-fluid">
        ...
        <!-- end page title -->
        @php $payment =
        App\Models\Payment::where('invoice_id',$invoice->id)->first(); @endphp
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4>
                            {{ __('Invoice No') }}: #{{ $invoice->invoice_no }} - {{
                            date('d-m-Y',strtotime($invoice->date)) }}
                        </h4>
                        <a
                            href="{{ route('invoice.pending.list') }}"
                            class="btn btn-dark btn-rounded waves-effect waves-light"
                            style="float: right"
                            ><i class="fa fa-list"> {{ __('Pending Invoice List') }} </i></a
                        >
                        <br />
                        <br />
                        ...
                        >
                            ...
                            >
                                ...
                                <tbody>
                                    ...
                                    @foreach($invoice['invoice_details'] as $key
                                    => $details)
                                    <tr>
                                        <input
                                            type="hidden"
                                    ...
                                    @php $total_sum += $details->selling_price;
                                    @endphp @endforeach
                                    <tr>
                                        <td colspan="6">{{ __('Sub Total') }}</td>
                                        <td>{{ $total_sum }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colspan="6">{{ __('Grand Amount') }}</td>
